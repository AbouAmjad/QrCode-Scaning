/** إعدادات مشتركة — AICS System (VPS + PostgreSQL) */
const AppConfig = {
  ...
  THEME_KEY: "uiTheme",
  THEME_COLOR_LIGHT: "#14532d",
  THEME_COLOR_DARK: "#06140a",
  SESSION_TOKEN: "",
   ...
   * - PAGE_PERMISSIONS = authorization gate for opening a page (and Backend uses its own checks).
   * - Sidebar layout / groups / favorites / search live in ui.js (NAV config only).
   * - Reordering the sidebar must NEVER add/remove/rename codes here.
   */
  PAGE_PERMISSIONS: {
...
const THEME_PRESETS = [
  { id: "aics", mode: "dark", labelKey: "themeAics", swatch: "#5cb83a", meta: "#06140a", color: "#14532d" },
...
function themeMeta(id = getTheme()) {
  return THEME_BY_ID[normalizeThemeId(id)] || THEME_BY_ID.aics;
}
...
function toggleTheme() {
  ...
  const next = isDarkTheme() ? "light" : "aics";
...
try {
  ...
  const earlyMeta = THEME_BY_ID[earlyId] || THEME_BY_ID.aics;
