/** Shared Excel / PDF export helpers for AbouAmjad Store System */
const TCExport = (() => {
...
<button class="noprint" onclick="window.print()" style="margin-bottom:12px;padding:8px 14px;font-weight:700;cursor:pointer">Print / Save PDF</button>
<h1>Not returned · AbouAmjad Store</h1>
...
<script>window.onload=function(){setTimeout(function(){window.print();},400);};</script>
  ...
  function exportPersonExcel(report) {
    ...
    const held = [
      ["Tool", "Description", "Qty", "Days out", "Taken at", "Overdue"],
...
<script>window.onload=function(){setTimeout(function(){window.print();},400);};</script>
  ...
  function exportProjectOnSiteExcel(project, holdings, label) {
    ...
    const aoa = [
      ...
      [],
      ["Code", "Description", "Qty on site"],
      ...list.map((h) => [h.code || "", h.description || "", h.qty != null ? h.qty : 1]),
      [],
      ["Total lines", list.length],
      ["Total qty", list.reduce((s, h) => s + (Number(h.qty) || 0), 0)]
    ];
    excelFromSheets(
      [{ name: "On site now", aoa, cols: [{ wch: 14 }, { wch: 36 }, { wch: 12 }] }],
...
<table>
  ...
  <tbody>${body || `<tr><td colspan="4">No tools currently on this project.</td></tr>`}</tbody>
</table>
${printCreditBlock()}
<script>window.onload=function(){setTimeout(function(){window.print();},400);};</script>
</body></html>`;
...
<button class="noprint" onclick="window.print()" style="margin-bottom:12px;padding:8px 14px;font-weight:700;cursor:pointer">Print / Save PDF</button>
<h1>Products catalog · AbouAmjad Store</h1>
...
<div class="top">
  <div class="brand">Store Request #${safe(req.id || "—")}<small>AbouAmjad · AICS</small></div>
  <div class="meta">
...
<script>
  ...
  function exportScrapExcel(items, label) {
    ...
    excelFromSheets(
      [{
        name: "Scrap",
        aoa: [["Code", "Description", "State", "Updated", "Note", "By"], ...rows],
        cols: [{ wch: 12 }, { wch: 36 }, { wch: 14 }, { wch: 20 }, { wch: 28 }, { wch: 14 }]
      }],
...
<button class="noprint" onclick="window.print()" style="margin-bottom:12px;padding:8px 14px;font-weight:700;cursor:pointer">Print / Save PDF</button>
<h1>Scrap list</h1>
<div class="sub">AbouAmjad Store System · ${safeHtml(label || stamp())} · ${list.length} item(s)</div>
<table>
  <thead><tr>
