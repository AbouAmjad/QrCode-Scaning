 ...
 *  opts.keeper — when set, only lots checked out by that store keeper (scans.created_by).
 */
function holdingsForPerson(ledger, personCode, opts = {}) {
  ...
  const keeperFilter = String(opts.keeper || "").trim().toLowerCase();
  ...
  for (const tool of ledger.tools.values()) {
    for (const lot of tool.lots) {
      ...
      if (keeperFilter) {
        const lotKeeper = String(lot.createdBy || "").trim().toLowerCase();
        if (lotKeeper !== keeperFilter) continue;
      }
