...
/**
 * Store keepers who only operate the terminal should see THEIR checkouts only.
 * Managers with inventory overview (or admin) see everyone.
 * Attribution = scans.created_by on each outstanding lot.
 */
function outstandingKeeperScope(ctx) {
  const role = String(ctx.role || "").trim().toLowerCase();
  if (role === "admin") return { mode: "all", keeper: "" };
  if (ctx.can("inventory.view") || ctx.can("users.manage") || ctx.can("roles.manage") || ctx.can("dashboard.full")) {
    return { mode: "all", keeper: "" };
  }
  const username = String((ctx.user && ctx.user.username) || "").trim();
  if (!username) return { mode: "all", keeper: "" };
  if (ctx.can("tool.return") || ctx.can("terminal.use") || ctx.can("tool.checkout")) {
    return { mode: "self", keeper: username };
  }
  return { mode: "all", keeper: "" };
}
...
async function getOutstanding(ctx) {
  ...
  const scope = outstandingKeeperScope(ctx);
  // Admin / managers may optionally narrow via ?keeper=username (UI filter).
  let keeperFilter = scope.mode === "self" ? scope.keeper : "";
  if (scope.mode === "all") {
    const requested = U.trimmed(ctx.params.keeper);
    if (requested) keeperFilter = requested;
  }
  ...
  const items = custody
    ...
    .map((personCode) => {
      const tools = custody.holdingsForPerson(ledger, personCode, {
        overdueDays,
        now,
      ...
      const keepers = [
        ...new Set(
          tools.flatMap((t) =>
            Array.isArray(t.keepers) && t.keepers.length
              ? t.keepers
              : t.keeper
                ? [t.keeper]
                : (t.lots || []).map((l) => l.keeper || l.createdBy || "").filter(Boolean)
          )
        ),
      ];
      return {
  ...
  return {
    ...
    scope: {
      mode: scope.mode,
      keeper: scope.mode === "self" ? scope.keeper : (keeperFilter || ""),
    },
    items,
    totals: {
      people: items.length,
...
async function getLifecycle(ctx) {
  ...
  const [r, meta] = await Promise.all([
    ctx.query(`SELECT * FROM tool_lifecycle ORDER BY id ASC`),
    ctx.query(
      `SELECT c.code,
              COALESCE(c.description, c.code) AS description,
              COALESCE(ph.image_url, '') AS image_url
         FROM catalog c
         LEFT JOIN product_photos ph ON UPPER(ph.code) = UPPER(c.code)
        WHERE c.kind <> 'person'`
    ),
  ]);
  ...
  r.rows.forEach((row) => {
    ...
    const m = metaOf.get(code) || {};
    latest.set(code, {
      code,
      description: m.description || code,
      imageUrl: m.imageUrl || "",
      state: row.state,
