...
const AppConfig = {
  ...
  /**
   * Per-user themes (localStorage). Each entry has a light|dark base so legacy
   * html.theme-dark / theme-light CSS rules keep working.
   */
  THEMES: [
    { id: "light", base: "light", labelKey: "themeLight", swatch: "#0f766e", meta: "#0f766e" },
    { id: "dark", base: "dark", labelKey: "themeDark", swatch: "#2dd4bf", meta: "#0b1220" },
    { id: "ocean", base: "dark", labelKey: "themeOcean", swatch: "#22d3ee", meta: "#031014" },
    { id: "midnight", base: "dark", labelKey: "themeMidnight", swatch: "#60a5fa", meta: "#030712" },
    { id: "forest", base: "dark", labelKey: "themeForest", swatch: "#34d399", meta: "#04140e" },
    { id: "graphite", base: "dark", labelKey: "themeGraphite", swatch: "#94a3b8", meta: "#0a0a0b" },
    { id: "sky", base: "light", labelKey: "themeSky", swatch: "#0284c7", meta: "#0284c7" },
    { id: "sand", base: "light", labelKey: "themeSand", swatch: "#78716c", meta: "#78716c" }
  ],
  SESSION_TOKEN: "",
  ...
  ROLE_PAGES: {
    ...
    ceo: ["dashboard", "projects", "warehouses", "damage", "logs", "forms", "qc", "timesheetAdmin", "profile"],
    accountant: ["dashboard", "projects", "warehouses", "damage", "forms", "timesheetAdmin", "profile"],
    hr: ["dashboard", "people", "forms", "timesheetAdmin", "profile"],
...
function listThemes() {
  return Array.isArray(AppConfig.THEMES) ? AppConfig.THEMES : [
...
function getThemeMeta(id) {
  ...
  return themes.find((t) => t.id === id) || themes.find((t) => t.id === "dark") || { id: "dark", base: "dark" };
}

function isThemeDark(id) {
  return getThemeMeta(id).base !== "light";
}

function getTheme() {
  const saved = localStorage.getItem(AppConfig.THEME_KEY);
  const known = listThemes().some((t) => t.id === saved);
...
function setThemeId(next) {
  ...
  localStorage.setItem(AppConfig.THEME_KEY, meta.id);
  applyTheme();
  syncThemeControls();
  return meta.id;
}
...
function applyTheme() {
  ...
  if (colorMeta) {
    colorMeta.setAttribute(
      "content",
      meta.meta || (isDark ? AppConfig.THEME_COLOR_DARK : AppConfig.THEME_COLOR_LIGHT)
    );
  }
}

/** Header quick toggle: dark-base ↔ light (keeps existing button behavior). */
function toggleTheme() {
  const next = isThemeDark(getTheme()) ? "light" : "dark";
  setThemeId(next);
}

function themeLabel() {
  const meta = getThemeMeta(getTheme());
  if (typeof t === "function" && meta.labelKey) return t(meta.labelKey);
  return meta.id;
}
...
function themeToggleText() {
  if (typeof t === "function") {
    return isThemeDark(getTheme()) ? t("themeLight") : t("themeDark");
  }
  return isThemeDark(getTheme()) ? "Light" : "Dark";
}
...
function syncThemeControls() {
  const isDark = isThemeDark(getTheme());
  ...
  document.querySelectorAll(".st-theme-btn[data-theme]").forEach((btn) => {
    btn.classList.toggle("active", btn.getAttribute("data-theme") === getTheme());
  });
}

/* Apply theme ASAP to avoid flash (before body/DOMContentLoaded) */
try {
  const saved = localStorage.getItem(AppConfig.THEME_KEY) || "dark";
  const lightIds = { light: 1, sky: 1, sand: 1 };
  const earlyDark = !lightIds[saved];
  const known = { light: 1, dark: 1, ocean: 1, midnight: 1, forest: 1, graphite: 1, sky: 1, sand: 1 };
  const earlyId = known[saved] ? saved : "dark";
