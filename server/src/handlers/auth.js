...
async function registerUser(ctx) {
  ...
  await query(
    ...
    [
      username,
      hash,
      permissions.normalizeRoleLocal(params.role),
      newToken(),
...
async function updateUser(ctx) {
  ...
  await ctx.audit({
    ...
    after: JSON.stringify({ username: target.username, changed: sets.length }),
  });
...
async function deleteUser(ctx) {
  ...
  if (!target) return { success: false, error: "NOT_FOUND" };
  if (target.username === ctx.user.username) return { success: false, error: "CANNOT_DELETE_SELF" };
  await ctx.query(`DELETE FROM users WHERE id = $1`, [target.id]);
  await ctx.audit({ action: "USER_DELETE", before: JSON.stringify({ username: target.username }) });
  return { success: true, username: target.username };
}
